...
export default function AdminLessonQuestionsPage() {
  ...
  const handleDragEnd = async (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (dragItem.current !== null && dragOverItem.current !== null && dragItem.current !== dragOverItem.current) {
      ...
      try {
        const res = await apiFetch(`/admin/lessons/${id}/questions/reorder/`, {
          method: "POST",
  ...
  const handleDelete = async (qId: number) => {
    ...
    try {
      const res = await apiFetch(`/admin/questions/${qId}/`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const res = await apiFetch(`/admin/questions/${editingId}/`, {
          method: "PATCH",
        ...
        const res = await apiFetch(`/admin/lessons/${id}/questions/`, {
          method: "POST",
          body: JSON.stringify(data)
        });
        if (res.ok) {
      ...
      <div className="space-y-4">
        {questions.length === 0 && !showAddForm && (
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500 mb-4">Ushbu dars uchun hali test savollari qo'shilmagan.</p>
            <button
              onClick={handleAddClick}
